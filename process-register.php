...
if(isset($_POST['username_account']) && isset($_POST['email_account']) && isset($_POST['password_account1']) && isset($_POST['password_account2'])){
    ...
    if(empty($username_account)){
        ...
        die(header('Location: form-register.php')); //คุณไม่ได้กรอกยืนยันรหัสผ่าน
        ...
        if(mysqli_num_rows($call_back_query_check_email_account) > 0){
            die(header('Location: form-register.php')); //มีผู้ใช้อีเมลแล้ว
        }else{
            $length = random_int(97,128);
            $salt_account = bin2hex(random_bytes($length)); //สร้างค่าเกลือ
            $password_account1 = $password_account1 . $salt_account; //เอารหัสผ่านต่อค่าเกลือแล้ว เข้าต่อด้วยวิธี ARGON2ID
            ...
            $password_account = password_hash($password_account1, $algo, $options); //นำรหัสผ่านที่ต่อค่าเกลือแล้ว
